...
def test_task_card_body_is_valid_markup(monkeypatch):
    ...
    captured: list[str] = []
    monkeypatch.setattr(TaskCard, "update",
                        lambda self, content="", **k: captured.append(content))
    t = Task(id=1, title=NASTY, status="todo", tags="x[/]y,z",
             estimated_pomodoros=2, position=0)
    TaskCard(t, project_name="p[/]roj", project_color="cyan",
