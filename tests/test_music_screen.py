...
def test_seek_absolute_rounds():
    mc = _enabled()
    with patch("pomodoro.music.shutil.which", return_value="/usr/bin/cliamp"), \
...
def test_shuffle_repeat_stop_argv():
    ...
    with patch("pomodoro.music.shutil.which", return_value="/usr/bin/cliamp"), \
         patch("pomodoro.music.subprocess.Popen") as popen:
        mc.shuffle("toggle"); assert _argv(popen) == ["cliamp", "shuffle", "toggle"]
        mc.repeat("cycle"); assert _argv(popen) == ["cliamp", "repeat", "cycle"]
        mc.stop(); assert _argv(popen) == ["cliamp", "stop"]
...
def test_speed_clamped():
    ...
    with patch("pomodoro.music.shutil.which", return_value="/usr/bin/cliamp"), \
         patch("pomodoro.music.subprocess.Popen") as popen:
        mc.speed(1.5); assert _argv(popen) == ["cliamp", "speed", "1.5"]
        mc.speed(9.0); assert _argv(popen) == ["cliamp", "speed", "2"]
        mc.speed(0.01); assert _argv(popen) == ["cliamp", "speed", "0.25"]
...
def test_transport_disabled_is_noop():
    ...
    with patch("pomodoro.music.subprocess.Popen") as popen:
        mc.seek(5); mc.shuffle(); mc.repeat(); mc.stop(); mc.speed(1.2); mc.load_playlist("x")
        assert not popen.called
...
def test_playlists_parses_text(monkeypatch):
    mc = _enabled()
    monkeypatch.setattr("pomodoro.music.shutil.which", lambda p: "/usr/bin/cliamp")
    cp = subprocess.CompletedProcess([], 0, stdout="• My Mix (12 tracks)\nChill\n1. Focus\n", stderr="")
    monkeypatch.setattr("pomodoro.music.subprocess.run", lambda *a, **k: cp)
...
def test_extract_nested_and_flags():
    out = music_view.extract({
        ...
        "state": "playing", "volume": -3, "shuffle": True, "repeat": "All",
    })
...
async def test_music_screen_enter_plays_playlist_and_track(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        async with app.run_test() as pilot:
            ...
            monkeypatch.setattr(scr.controller, "load_playlist",
                                lambda name, **k: calls.__setitem__("load", name))
            monkeypatch.setattr(scr.controller, "play_track",
                                lambda path: calls.__setitem__("track", path))
