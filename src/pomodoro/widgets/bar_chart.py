...
def render_bars(data: list[tuple[str, int]], width: int = 30,
...
class BarChart(Static):
    ...
    def set_data(self, data: list[tuple[str, int]], width: int = 30,
                 colors: list[str] | None = None, value_suffix: str = "") -> None:
        self.update(render_bars(data, width=width, colors=colors, value_suffix=value_suffix))


def render_vertical_bars(data: list[tuple[str, int]],
                          height: int = 6, color: str = "cyan") -> str:
...
class VerticalBarChart(Static):
    ...
    def set_data(self, data: list[tuple[str, int]], height: int = 6,
                 color: str = "cyan") -> None:
