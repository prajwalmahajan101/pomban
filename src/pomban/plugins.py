...
Plugins are Python entry points under group `pomban.hooks` exposing callables:
...
they're logged to ~/.local/state/pomban/plugins.log.
"""
...
class PluginRegistry:
    ...
    def discover(self) -> None:
        try:
            eps = entry_points(group="pomban.hooks")
        except TypeError:
            # Older importlib.metadata API
            eps = entry_points().get("pomban.hooks", [])
        for ep in eps:
            try:
                obj = ep.load()
...
def git_sync(repo_dir: Path | str) -> None:
    ...
    if not (repo / ".git").exists():
        return
    msg = f"pomban sync {datetime.now().isoformat(timespec='seconds')}"
